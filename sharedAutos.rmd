...
week_hour_avgs %>% qplot(b_i, geom ="histogram", data = ., color = I("slategray4"))
...
model_2_day_rmse <- RMSE(predicted_ratings_week, db_week_test$total_cars)
...
predicted_ratings <- mu_Train + db_test %>% 
  left_join(week_hour_avgs, by='Hour') %>%
  pull(b_i)
...
day_grid_avgs <- db_day_train %>% 
  left_join(day_hour_avgs, by = 'Hour') %>%
  group_by(gridKey) %>% 
  summarize(b_u = mean(total_cars - mu_db_dayTrain - b_i))

predicted_ratings_day <- db_day_test %>% 
  left_join(day_hour_avgs, by='Hour') %>%
  left_join(day_grid_avgs, by='gridKey') %>%
  mutate(pred = mu_db_dayTrain + b_i + b_u) %>%
  pull(pred)
...
fit_dayW <- lm(total_cars ~ as.factor(Weekday), data = db_day_train)
...
pred_day <- lm(total_cars ~ Hour + gridKey, data = db_day_train)
...
train_knn <- train(total_cars ~ Hour + gridKey + Weekday, method = "knn", data = db_train)

y_hat_knn <- predict(train_knn, db_test, type = "raw")

plot(train_knn)

train_knn$bestTune

RMSE(y_hat_knn, db_test$total_cars)
...
RMSE(y_hat_knnD, db_day_test$total_cars)
...
RMSE(y_hat_knnW, db_week_test$total_cars)
